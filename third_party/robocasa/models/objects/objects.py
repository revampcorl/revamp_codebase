...
class MJCFObject(MujocoXMLObjectRobocasa):
    ...
    def __init__(
        ...
        tmp_folder = os.environ.get("ROBOCASA_MJCF_TMPDIR", folder)
        if tmp_folder != folder:
            os.makedirs(tmp_folder, exist_ok=True)
            xml_str = self.absolutize_asset_paths(xml_str, folder)
        new_xml_path = os.path.join(tmp_folder, "{}_{}.xml".format(time_str, os.getpid()))
    ...
    def absolutize_asset_paths(self, xml_str, folder):
        """Make relative mesh/texture paths robust when temp XML lives elsewhere."""
        ...
        for elem in asset.findall("mesh") + asset.findall("texture"):
            ...
            if old_path is None or os.path.isabs(old_path):
                continue
            elem.set("file", os.path.abspath(os.path.join(folder, old_path)))
